...
namespace PoeHUD.Hud.Preload
{
    public class PreloadAlertPlugin : SizedPlugin<PreloadAlertSettings>
    {
        ...
        public override void Render()
        {
            ...
            foreach (Size2 size in alerts.Select(preloadConfigLine => Graphics.DrawText(preloadConfigLine.Text, Settings.FontSize, position + 1, preloadConfigLine.FastColor?.Invoke() ?? preloadConfigLine.Color ?? Settings.DefaultFontColor, FontDrawFlags.Right)))
            {
