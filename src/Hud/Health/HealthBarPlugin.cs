    ...
    public class HealthBarPlugin : Plugin<HealthBarSettings>
    {
        ...
        public override void Render()
        {
            ...
            foreach (HealthBar healthBar in healthBars.SelectMany(x => x.Value).AsParallel().AsOrdered().Where(hp => showHealthBar(hp) && hp.Entity.IsAlive))
            {
                ...
                if (mobScreenCoords == new Vector2()) continue;
        ...
        private float DrawDebuffPanel(Vector2 startPoint, HealthBar healthBar, Life life)
        {
            ...
            if (buffs.Count <= 0) return startY;
            var isHostile = healthBar.Entity.IsHostile;
            int debuffTable = 0;
            foreach (var buffName in buffs.Select(buff => buff.Name))
            {
            ...
            if (debuffTable <= 0) return startY;
        ...
        private static bool HasDebuff(Dictionary<string, int> dictionary, string buffName, bool isHostile)
        {
            int filterId;
        ...
        private float DrawDebuff(Func<bool> predicate, float startX, float startY, int index, float marginFix = 0f)
        {
            ...
            const float oneIconWidth = 1.0f / ICON_COUNT;
            if (marginFix > 0)
        ...
        {
            if (!settings.ShowPercents) return;
